...
# 🔥 BLOCO ÚNICO DE STATUS (CONSOLIDADO)
def definir_status(row, chamada_atual, chamada_fechada):
    status_planilha = str(row["Situação do requerimento de matrícula"]).strip()
    proc = row["Processo seletivo"]
    chamada = row["Chamada"]

    # 1. Prioridade absoluta: status explícito da planilha
    if status_planilha in MAPA_STATUS:
        return MAPA_STATUS[status_planilha]

    # 2. Sem chamada → lista de espera
    if chamada == 0:
